...
def gen_arti(centerx=1,centery=1,sigma=0.1,nbex=1000,data_type=0,epsilon=0.02):
    ...
    if data_type==3:
        #cercle au centre
        data=np.reshape(np.random.uniform(-4,4,2*nbex),(nbex,2))
        y=np.array([ 1 if data[i,0]**2 + data[i,1]**2 < 1 else -1 for i in range(data.shape[0])])
    if data_type==4:
        #Plusieurs cercles
        nc = np.reshape(np.random.uniform(-4,4,2*4),(4,2))
        data=np.reshape(np.random.uniform(-4,4,2*nbex),(nbex,2))
        y=np.array([ 1 if (data[i,0] - nc[0,0])**2 + (data[i,1] - nc[0,1])**2 < 1 
                    or (data[i,0] - nc[1,0])**2 + (data[i,1] - nc[1,1])**2 < 1 
                    or (data[i,0] - nc[2,0])**2 + (data[i,1] - nc[2,1])**2 < 1 
                    or (data[i,0] - nc[3,0])**2 + (data[i,1] - nc[3,1])**2 < 1
